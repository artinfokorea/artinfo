...
// export default function ConcertCard({ item, isLastPage }: IProps) {
...
const ConcertCard = forwardRef<HTMLDivElement, IProps>(
  ({ item, isLastPage }, ref) => {
    const filters = useFilters()
    const [isLoading, setIsLoading] = useState(true)
    const date = filters.DIFF_FROM_NOW_ADD_TIME(
    ...
    return (
      <div className="card flex flex-col bg-zinc-900 rounded-md" ref={ref}>
        <div className="relative h-[220px] lg:h-[300px]">
          {isLoading && (
            <div className="flex items-center justify-center absolute inset-0">
              <Spinner />
